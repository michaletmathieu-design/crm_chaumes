...
export default function UploadDocuments({ prospectId, initialDocs }: { prospectId: string, initialDocs: Doc[] }) {
  ...
  async function handleUpload(file: File) {
    ...
    formData.append("targetType", "PROSPECT");
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDocs(prev => [{ id: Date.now().toString(), name: file.name, fileUrl: "pending", category: "CONTRAT" }, ...prev]);
      } else {
  ...
  return (
    <div>
      ...
      >
        ...
        />
        <label htmlFor="file-upload-prospect" className="cursor-pointer flex flex-col items-center gap-2">
